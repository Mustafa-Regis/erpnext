...
def get_data(item, warehouse):
	...
	stock_details_map = get_stock_details_map(variant_list,  warehouse=warehouse)  # Include warehouse parameter
	...
	for item_dict in variant_dicts:
		...
		if stock_details_map.get(name):
			stock_details = stock_details_map.get(name).get(warehouse, {})  # Include warehouse parameter
			item_dict["current_stock"] = stock_details.get("Inventory") or 0
			item_dict["in_production"] = stock_details.get("In Production") or 0
		else:
...
def get_stock_details_map(variant_list, warehouse=None):
	stock_details = frappe.db.get_all(
		...
		filters={"item_code": ["in", variant_list], "warehouse": warehouse},
	)
	...
	for row in stock_details:
		...
		if not stock_details_map.get(name):
			stock_details_map[name] = {}

		stock_details_map[name][warehouse] = {
