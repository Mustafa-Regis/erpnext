...
def get_periodic_data(entry, filters):
	...
	for d in entry:
		...
		if filters["value_quantity_pricelist"] == "Value":
...
def get_data(filters):
	...
	ranges = get_period_date_ranges(filters)
	price_list = filters.get("price_list")  # Assuming the price list is provided in the filters
	...
	for dummy, item_data in item_details.items():
		...
		for start_date, end_date in ranges:
			...
			if period_data:
				...
				if filters["value_quantity_pricelist"] == "Pricelist":
					row[scrub(period)] = calculate_value(row[scrub(period)], item_data.name, price_list)
			else:
				row[scrub(period)] = previous_period_value if today >= start_date else None
				if filters["value_quantity_pricelist"] == "Pricelist" and row[scrub(period)] is not None:
					row[scrub(period)] = calculate_value(row[scrub(period)], item_data.name, price_list)
...
def get_chart_data(columns, data):
	...
	column_titles = list(data[0].keys())[5:]

	 # بناء مصفوفة لتخزين مجموع القيم لكل عمود
	column_sums = [round(sum(row[title] for row in data if row[title] is not None)) for title in column_titles]


	chart = {'data': {'labels': labels, "datasets":[{'values':column_sums}]}}
	chart["type"] = "bar"
...
def calculate_value(quantity, item_code, price_list):
    # Fetch the price for the given item code and price list
    item_price = frappe.db.get_value("Item Price", {"item_code": item_code, "price_list": price_list}, "price_list_rate")

    if item_price is not None:
        # Calculate the value based on the quantity and price
