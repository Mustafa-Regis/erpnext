...
from erpnext.stock.report.stock_based_on_price_list.stock_based_on_price_list import execute, get_period_date_ranges

def stock_based_on_price_list(filters):
    col, data, *_ = execute(filters)
    return col, data

class TestStockBasedOnPriceListReport(FrappeTestCase):
    def setUp(self) -> None:
    ...
    def assert_single_item_report(self, movement, expected_buckets):
        ...
        cols, data = stock_based_on_price_list(filters)
